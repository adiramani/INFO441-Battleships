...
func NewSessionID(signingKey string) (SessionID, error) {
	if len(signingKey) == 0 {
		return InvalidSessionID, errors.New("cannot have an empty signing key")
	}

	randoByte := make([]byte, idLength)
	_, err := rand.Read(randoByte)
	if err != nil {
		return InvalidSessionID, errors.New("error generating random bytes")
	}

	h := hmac.New(sha256.New, []byte(signingKey))
	h.Write(randoByte)
	byteSlice := h.Sum(randoByte)

	sID := SessionID(base64.URLEncoding.EncodeToString(byteSlice))
